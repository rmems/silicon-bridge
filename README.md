...
A silicon-bridge UART host session on this board is the crates.io 0.3.0
publish gate ([#84](https://github.com/rmems/silicon-bridge/issues/84)).
...
in-tree `0.3.0` string is not a live registry crate. `[package].version`
is `0.3.0`; an authorized `cargo publish` is **held** until the Basys 3
host path is proven
([#84](https://github.com/rmems/silicon-bridge/issues/84)). Until then,
depend on git or a path:
...
(intended first release: **0.3.0**, after #84):
...
**0.3.0** package candidate (`[package].version` is `0.3.0`). An authorized
human `cargo publish` is **held** until the Basys 3 UART host path is proven
([#84](https://github.com/rmems/silicon-bridge/issues/84)). Do not treat the
README badge or crates.io install snippets as a live registry crate.
