...
| 9 | A library writes 20 lines to stdout on every export | `src/fpga_export.rs`, `src/fpga_bridge.rs` | Medium | **#50** (export report; bridge already silent in #51) |
...
`MemFileWriter::write_mem_files`. #50 returns an `ExportReport` instead and
does not print. `FpgaBridge` constructors already stopped printing in #51.

**Fix (#50)** — profiled `write_with_config` / legacy `write_mem_files` return
`ExportReport`. No unsolicited stdout on the default library export path.
